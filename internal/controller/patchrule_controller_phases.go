...
func (r *PatchRuleReconciler) reconcileDelete(ctx context.Context, rule *v1.PatchRule) (ctrl.Result, error) {
	...
	for key, state := range rule.Status.Targets {
		...
		log.V(1).Info("reverted", "target", key)
	}
	return ctrl.Result{}, nil
}
...
func (r *PatchRuleReconciler) reconcileNormal(ctx context.Context, rule *v1.PatchRule) (ctrl.Result, error) {
	log := log.FromContext(ctx)
	...
	conflictRule, conflictPaths, err := r.checkConflicts(ctx, rule, gvk, matchedKeys)
	if err != nil {
		return ctrl.Result{}, err
	}
	if conflictRule != "" {
		...
		log.Info("conflict detected", "conflictsWith", conflictRule, "paths", conflictPaths)
	...
	newTargets := make(map[string]v1.TargetState, len(matched))
...
func (r *PatchRuleReconciler) reconcilePatch(
	...
	rule *v1.PatchRule,
	m matchedTarget,
	oldTargets map[string]v1.TargetState,
) (v1.TargetState, error) {
	log := log.FromContext(ctx)

	// Capture original values BEFORE patching (target.Object is the pre-patch snapshot)
	priorValues := captureOverwrittenValues(
		m.target.Object, rule.Spec.Additions.Data, rule.Spec.Overwrite)
	removedEntries := captureRemovedEntries(
		m.target.Object, rule.Spec.Removals.Data)

	patch := buildPatch(m.target, rule)
	if patch != nil {
		patchBytes, err := json.Marshal(patch)
		if err != nil {
			return v1.TargetState{}, fmt.Errorf("marshal patch: %w", err)
		}
		if err := r.Patch(ctx, m.target,
			client.RawPatch(types.MergePatchType, patchBytes),
			client.FieldOwner(v1.FieldManager)); err != nil {
			log.Error(err, "patch failed",
				"namespace", m.target.GetNamespace(), "name", m.target.GetName())
			return v1.TargetState{}, err
		}
		log.V(1).Info("patched",
			"namespace", m.target.GetNamespace(), "name", m.target.GetName())
	}

	appliedAdditions := captureAppliedAdditions(
		m.target.Object, rule.Spec.Additions.Data, rule.Spec.Overwrite)

	// Merge with old prior values to preserve true originals across reconciles
	if oldState, ok := oldTargets[m.key]; ok {
		priorValues = mergePriorValues(oldState.PriorValues.Data, priorValues)
		removedEntries = mergePriorValues(oldState.RemovedEntries.Data, removedEntries)
	}

	return v1.TargetState{
		AppliedAdditions: v1.NestedObject{Data: appliedAdditions},
		PriorValues:      v1.NestedObject{Data: priorValues},
		RemovedEntries:   v1.NestedObject{Data: removedEntries},
	}, nil
}
...
func (r *PatchRuleReconciler) revertStaleTargets(
	...
	oldTargets, newTargets map[string]v1.TargetState,
) error {
...
func (r *PatchRuleReconciler) revertDroppedSpecEntries(
	...
	for key, oldState := range oldTargets {
		...
		diffPatch := buildDiffRevertPatch(
...
func (r *PatchRuleReconciler) applyRevertPatch(
	...
	gvk schema.GroupVersionKind,
	key string,
	state v1.TargetState,
) error {
	revertPatch := buildRevertPatch(
